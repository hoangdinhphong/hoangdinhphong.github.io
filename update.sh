...
printf "Origin: Hoàng Đình Phong\nLabel: Hoàng Đình Phong\nSuite: stable\nVersion: 1.1\nCodename: ios\nArchitectures: iphoneos-arm iphoneos-arm64\nComponents: main\nDescription: Hoàng Đình Phong\nSHA256:\n "$(cat ./Packages | md5sum | cut -d ' ' -f 1)" "$(stat ./Packages --printf="%s")" Packages\n "$(cat ./Packages.bz2 | md5sum | cut -d ' ' -f 1)" "$(cat ./Packages.xz | md5sum | cut -d ' ' -f 1)" "$(cat ./Packages.gz | md5sum | cut -d ' ' -f 1)" "$(stat ./Packages.bz2, Packages.xz, Packages.gz --printf="%s")" Packages.bz2\nPackages.xz\nPackages.gz\n">Release;
